...
void Calibration_LSI(Cali_LevelTypeDef cali_Lv)
{
    ...
    while(1)
    {
        ...
        while(1)
        {
            ...
            if(((cnt_offset > -46 * (freq_sys / 1000) / 80000) && (cnt_offset < 46 * (freq_sys / 1000) / 80000)) || retry > 2)
            {
                if(retry)
                    break;
            }
            retry++;
            cnt_offset = (cnt_offset > 0) ? (((cnt_offset * 2) / (92 * (freq_sys/1000) / 80000)) + 1) / 2 : (((cnt_offset * 2) / (92 * (freq_sys/1000) / 80000 )) - 1) / 2;
            sys_safe_access_enable();
        ...
        cnt_offset = (cnt_offset > 0) ? ((((cnt_offset * 32000) / (14575 * ((1 << ((cali_Lv<=Level_128)?cali_Lv+2:(cali_Lv+2)))) * ((freq_sys/1000) / 80000))) +1) ) : ((((cnt_offset * 32000) / (14575 * ((1 << ((cali_Lv<=Level_128)?cali_Lv+2:(cali_Lv+2)))) * ((freq_sys/1000) / 80000)))-1) );
