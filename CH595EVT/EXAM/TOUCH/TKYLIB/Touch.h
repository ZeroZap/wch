...
#define KEY_REPEAT_TIME   0//100         //按键连发的速度，0表示不支持连发，单位为按键扫描次数

// #define KEY_PRESS_TIMEOUT_ENABLE         //使能按键长按时无效功能，使能后按键长按超时无效，超时后刷新基线
                                         //不使能则正常使用长按键功能，若想长按键功能应屏蔽此项

#define KEY_LONG_TIME     0           //按键长按超时时间，单位为按键扫描次数，长按时间 0 表示不检测长按键事件。
...
extern const uint32_t TKY_Pin[14][2];
/* 供外部调用的函数声明 */
...
extern void touch_Recalibrate_Wakeup( void );
extern void touch_Recalibrate_Sleep( void );
extern void touch_ResetBaselineWakeup (void);
extern void touch_ResetBaselineSleep (void);
#endif
