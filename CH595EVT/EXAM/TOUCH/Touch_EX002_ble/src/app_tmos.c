...
#define TRIGGER_TIME MS1_TO_SYSTEM_TIME(500)       // 500ms
...
void PeriodicDealData(void)
{
    ...
    else //---休眠状态时，醒来间隔进行扫描---
    {
        ...
        scandata = TKY_ScanForWakeUp(); //---对所选择的队列通道进行扫描---
        touch_Recalibrate_Sleep();
        if (scandata == 1) //---如扫描有异常，则调用正式扫描函数模式3~4---
        {
            for (uint8_t i = 0; i < (1<<(TKY_FILTER_GRADE+1)); i++)
            {
                keydata = TKY_PollForFilterSleep();
