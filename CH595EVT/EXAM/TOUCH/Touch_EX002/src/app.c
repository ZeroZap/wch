...
volatile uint16_t recal_time = 0;
...
#define adcSamp_cnt  22 //次数
volatile unsigned char adclen = 0;
unsigned int adcBuff[adcSamp_cnt] = {0};
static unsigned int battary_value = 0;
void BAT_ADC_Check(void)
{
    unsigned int ad_avg = 0;
    unsigned char i = 0;
    unsigned int battary_percent = 0;;
    GPIOA_ModeCfg(TKY_Pin[TKY_CX_CH_IDX][1], GPIO_ModeOut_PP_20mA);
    GPIOA_ResetBits(TKY_Pin[TKY_CX_CH_IDX][1]);
    ADC_ExtSingleChSampInit(SampleFreq_8, ADC_PGA_0);  
    GPIOA_ModeCfg(TKY_Pin[TKY_CX_CH_IDX][1], GPIO_ModeIN_Floating);
    ADC_ChannelCfg( TKY_CX_CH_IDX ); 
    DelayMs(3); //
    // ADC_ExcutSingleConver(); // 时间足够时建议再次转换并丢弃首次ADC数据
    ad_avg = ADC_ExcutSingleConver(); // 时间足够时建议再次转换并丢弃首次ADC数据

    battary_value = ADC_VoltConverSignalPGA_0dB(ad_avg);

    PRINT("ad_avg %d battary_value %d \n",ad_avg,battary_value);
}
/*********************************************************************
 * @fn      TKY_dataProcess
 *
...
void Timer_Init (void) {
    TMR0_TimerInit(FREQ_SYS/1000);               //定时周期为5ms
