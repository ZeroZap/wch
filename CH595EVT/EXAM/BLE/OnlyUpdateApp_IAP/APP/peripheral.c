 ...
 * GLOBAL VARIABLES
 */
volatile uint8_t PromLastPacket = 0;

__attribute__((aligned(4))) unsigned char Verify_buf[300];
RingMemParm_t RingMemPROM;
uint8_t       RingMemPROMBuf[2048];
/*********************************************************************
 * EXTERNAL VARIABLES
 */
...
void Peripheral_Init()
{
    ...
    RingMemInit(&RingMemPROM, RingMemPROMBuf, 2048, RingMem_Protection);
}
...
//        PRINT("ERASE:%08x num:%d\r\n", (int)(EraseAdd + EraseBlockCnt * FLASH_BLOCK_SIZE), (int)EraseBlockCnt);
        status = FLASH_ERASE(EraseAdd + EraseBlockCnt * FLASH_BLOCK_SIZE, FLASH_BLOCK_SIZE);
    ...
    if(events & OTA_FLASH_PROM_EVT)
    {
        ...
        if(RingMemPROM.CurrentLen > FLASH_PAGE_SIZE)
        {
            if(RingMemPROM.CurrentLen > FLASH_PAGE_SIZE)
            {
                RingMemCopy(&RingMemPROM, buf, FLASH_PAGE_SIZE);
                if( FLASH_WRITE(OpromAdd, buf, FLASH_PAGE_SIZE))
                {
                    tmos_start_task(Peripheral_TaskID, OTA_FLASH_PROM_EVT, 2);
                }
                OpromAdd+=0x100;
                RingMemDelete(&RingMemPROM, FLASH_PAGE_SIZE);
            }
        }
        if(PromLastPacket )
        {
            RingMemCopy(&RingMemPROM, buf, RingMemPROM.CurrentLen);
            if(FLASH_WRITE(OpromAdd , buf, RingMemPROM.CurrentLen))
            {
                tmos_start_task(Peripheral_TaskID, OTA_FLASH_PROM_EVT, 2);
            }
            RingMemDelete(&RingMemPROM, RingMemPROM.CurrentLen);
            PromLastPacket = 0;
        }

        tmos_set_event(Peripheral_TaskID, OTA_FLASH_PROM_EVT);
...
void SwitchImageFlag(uint8_t new_flag)
{
    ...
    /* 读取第一块 */
    FLASH_READ(OTA_DATAFLASH_ADD, (uint32_t *)&block_buf[0], 4);

    /* 擦除第一块 */
    FLASH_ERASE(OTA_DATAFLASH_ADD, FLASH_PAGE_SIZE);

    /* 更新Image信息 */
    block_buf[0] = new_flag;

    /* 编程DataFlash */
    FLASH_WRITE(OTA_DATAFLASH_ADD, (uint32_t *)&block_buf[0], 4);
}
...
//            PRINT("IAP_PROM: %08x len:%d \r\n", (int)OpAdd, (int)OpParaDataLen);
            ...
            /* 当前是ImageA，直接编程 */
            status = RingMemWrite(&RingMemPROM, iap_rec_data.program.buf, OpParaDataLen);
            tmos_set_event(Peripheral_TaskID, OTA_FLASH_PROM_EVT);
            OTA_IAP_SendCMDDealSta(status);
...
//            PRINT("IAP_VERIFY: %08x len:%d \r\n", (int)OpAdd, (int)OpParaDataLen);
            ...
            FLASH_READ(OpAdd, Verify_buf, OpParaDataLen);
            status = memcmp(Verify_buf , iap_rec_data.verify.buf, OpParaDataLen);
            VerifyStatus |= status;
