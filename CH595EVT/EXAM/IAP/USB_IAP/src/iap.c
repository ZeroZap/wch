...
__attribute__((aligned(4)))   uint8_t g_write_buf_check[256 + 64]; //每次满256字节再写flash，提升速度
...
void myDevEP2_OUT_Deal(uint8_t l)
{
    ...
    case CMD_IAP_PROM:
        if (g_iap_cmd.program.len == 0)
        {
            if (g_buf_write_ptr != 0)
            {
                ...
                s = FLASH_WRITE(g_flash_write_ptr, (PUINT32)g_write_buf, g_buf_write_ptr);
                g_buf_write_ptr = 0;
            }
        }
        else
        {
            ...
            {
                s = FLASH_WRITE(g_flash_write_ptr, (PUINT32)g_write_buf, 256);
    ...
    case CMD_IAP_ERASE:
    	...
    	{
    	    s = FLASH_ERASE(APP_CODE_START_ADDR, APP_CODE_END_ADDR - APP_CODE_START_ADDR);
    ...
    case CMD_IAP_VERIFY:
		...
		FLASH_READ(addr, g_write_buf_check, g_iap_cmd.verify.len);
		s = memcmp(g_write_buf , g_write_buf_check, g_iap_cmd.verify.len);
        myDevEP2_IN_Deal(s);
        break;
    case CMD_IAP_END:
        /*结束升级，复位USB，跳转到app*/
        R8_USB_CTRL = RB_UC_RESET_SIE;
        R16_PIN_ANALOG_IE &= ~(RB_UDP_PU_EN | RB_PIN_USB_IE);
        DelayMs(10);
        jumpApp();
        break;
    default:
