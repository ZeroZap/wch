...
__attribute__((aligned(4)))                    uint8_t g_write_buf_check[256 + 64];
                ...
                if (check_num_rec == check_num)
                {
                    ...
                    case CMD_IAP_PROM:
                        ...
                        if (g_update_permition == 1)
                        {
                            if (iap_rec_data.program.len == 0)
                            {
                                ...
                                {
                                    if (FLASH_WRITE(g_flash_write_ptr, (PUINT32)g_write_buf, g_buf_write_ptr))
                                    {
                                        iap_rsp_data[2] = 0xfe;
                            ...
                            else
                            {
                                ...
                                {
                                    /* 满256字节写一次 */
                                    if (FLASH_WRITE(g_flash_write_ptr, (PUINT32)g_write_buf, 256))
                                    {
                                        iap_rsp_data[2] = 0xfe;
                                        iap_rsp_data[3] = IAP_ERR_WRITE_FAIL;
                                        break;
                    ...
                    case CMD_IAP_ERASE:
                        if (iap_rec_data.erase.addr == APP_CODE_START_ADDR)
                        {
                            ...
                            if (FLASH_ERASE(APP_CODE_START_ADDR, APP_CODE_END_ADDR - APP_CODE_START_ADDR))
                            {
                                /* 擦除失败 */
                                iap_rsp_data[2] = 0xfe;
                    ...
                    case CMD_IAP_VERIFY:
                        if (((iap_rec_data.verify.addr % 4) == 0) && (iap_rec_data.verify.addr >= APP_CODE_START_ADDR) && (iap_rec_data.verify.addr < APP_CODE_END_ADDR))
                        {
                            ...
                            FLASH_READ(iap_rec_data.verify.addr, g_write_buf_check, iap_rec_data.verify.len);
                            if (memcmp(g_write_buf , g_write_buf_check, iap_rec_data.verify.len))
                            {
                                /* 校验失败，报错 */
                                iap_rsp_data[2] = 0xfe;
