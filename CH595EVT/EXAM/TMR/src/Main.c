...
int main()
{
    ...
    TMR3_PWMCycleCfg(FREQ_SYS / 10000); // 周期 100us  最大67108864
    TMR3_PWMActDataWidth(FREQ_SYS / 10000 / 2); // 占空比 50%, 修改占空比必须暂时关闭定时器
    TMR3_PWMEnable();
    TMR3_Enable();
    ...
    PRINT("TMR2 DMA PWM\n");
    TMR2_PWMCycleCfg(FREQ_SYS / 500); // 周期 2000us
    for(i=0; i<50; i++)
    {
       PwmBuf[i]=(FREQ_SYS / 500 / 50) * i;
    }
    for(i=50; i<100; i++)
    {
       PwmBuf[i]=(FREQ_SYS / 500 / 50)*(100-i);
    }
