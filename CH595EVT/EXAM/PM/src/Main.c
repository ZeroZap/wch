...
int main()
{
...
#if 0
    //如果使能了外部32K，可在稳定后配置成额定电流以降低功耗
    if(R8_CK32K_CONFIG & RB_CLK_OSC32K_XT)
    {
...
#if 1
    //关闭所有外设时钟，可降低IDLE模式功耗
    PWR_PeriphClkCfg(DISABLE,BIT_SLP_CLK_ALL&(~(BIT_SLP_CLK_UART1|BIT_SLP_CLK_RAMX|BIT_SLP_CLK_RAM2K)));//关闭串口1和RAM之外的外设时钟
    sys_safe_access_enable();
    R8_SLP_CLK_OFF2 |= 0x12;
    ...
    DelayMs(2);
    // 注意Sleep睡眠唤醒中断不可调用flash内代码。
    LowPower_Sleep(RB_PWR_RAM24K | RB_PWR_RAM8K | RB_XT_PRE_EN); //只保留24+8K SRAM 供电
    HSECFG_Current(HSE_RCur_100); // 降为额定电流(低功耗函数中提升了HSE偏置电流)
    PRINT("wake.. \n");
    DelayMs(500);
#endif
