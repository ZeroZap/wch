...
void PD_User_Dev_Attatched( u8 ch )
{
	...
	{   // Src access, wait for SrcCap
        ...
        //Sink access
        {
            ...
            PORT[PHYch].PHY.txSrcCapCnt = SrcCapCnt;
            memset( PORT[PHYch].PHY.txSrcCap, 0, PORT[PHYch].PHY.txSrcCapCnt );		
            memcpy( PORT[PHYch].PHY.txSrcCap, &SrcCap, PORT[PHYch].PHY.txSrcCapCnt*4 );
...
void PD_User_Src_Tx_PS_RDY (void)
{
    ...
    /* After Port0 PD is established, send GetSinkcap message to obtain the power supply message capability of SNK */
    fGetSinkCap = 1;
    tGetSinkCap = 60;
...
void PD_User_Src_Rx_SinkCap (void)
{
    ...
    for (u8 i = 0; i < rxHeader[PHYch]->NDO; i++) 
    {
        ...
        } else if ((rxSinkCap[i] >> 28) == 0xC) { /* APDO:PPS */
...
void PD_User_Timer (void)
{
    ...
    /* Port0 PD建立后，发送GetSinkcap消息获取SNK的供电消息能力 */
    if (fGetSinkCap) {

        tGetSinkCap--;
        if (!tGetSinkCap) {

            if (PORT[PHYch].PHY.WaitMsgTx || PORT[PHYch].PHY.WaitMsgRx)
                tGetSinkCap = 5;
            else {
