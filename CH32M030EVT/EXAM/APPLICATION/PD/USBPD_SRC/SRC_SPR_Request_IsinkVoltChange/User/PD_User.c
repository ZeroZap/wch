...
void PD_User_ChangeVoltTypeSet(void)
{
    ...
    PORT[PHYch].PHY.ChangeVoltType =  PD_VC_DCDCISink;	
}
...
void PD_User_Dev_Attatched( u8 ch )
{
	...
	{   // Src access, wait for SrcCap
            ...
            C0_DCDC_IN_VBUS_EN;		/* Open DCDC input */
...
void PD_User_Dev_Unattatched( u8 ch )
{
	...
	(!ch)?(C0_DCDC_OUT_VBUS_DS):(NULL);/* Close DCDC input output */
}
...
void PD_User_Emk_Rx_Disc_Ident (void)
{
    ...
    if(PORT[PHYch].DEVICE.EmarkerMaxCurr == 2)   /* 5A */
        SrcCap_5_20V[4] = SPR_Fixed_PDO(20, 5, 0);    /* 100W */

    Set_TXSrcCap(PHYch, SrcCap_5_20V, SrcCapCnt);
}
...
u32 PD_User_Snk_Tx_Request (void)
{
    ...
    if (PHYch)
        ObjPos = PORT[PHYch].PHY.rxSrcCapFixedMaxPos; /* 请求20V */
