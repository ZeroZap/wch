...
* Version            : V1.2
* Date               : 2024/07/10
* Description        : Simulate the program of a USB flash drive (EMMC as storage medium)
*******************************************************************************
...
/******************************************************************************/
/* MODE_SENSE information ,CMD 0X1A */
...
/******************************************************************************/
/* MODE_SENSE information,CMD 0X5A */
...
volatile uint8_t  Udisk_Status = 0x00;                                                              /* UDisk related status */
volatile uint8_t  Udisk_Transfer_Status = 0x00;                                                     /* UDisk transmission related status  */
volatile uint32_t Udisk_Capability = 0x00;                                                          /* UDisk formatted Capacity */
...
* Description    : Current UDisk Command Execution Status
* Input          : key------Primary key for disk error details information
*                  asc------Secondary key for disk error details information
...
* Description    : Current USB command execution failed
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
* Description    : Preparation before processing read and write sectors
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
{
    /* save the sector number that you currently want to work on */
    ...
    /* saves the current length of data to be manipulated */                    
...
void UDISK_SCSI_CMD_Deal( void )
{
      ...
      &&( mBOC.mCBW.mCBW_Sig[ 2 ] == 'B' ) && ( mBOC.mCBW.mCBW_Sig[ 3 ] == 'C' ) )
    {
        ...
        /* the udisk drive scsi command package is processed */ 
    ...
    {   /* the package logo of the cbw package is wrong */
...
* Description    : usb flash drive upload endpoint processing
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
* Description    : udisk downlink endpoint processing
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
{
    uint32_t i;
    /* endpoint 2 down to back data processing */
    if( Udisk_Transfer_Status & DEF_UDISK_BLUCK_DOWN_FLAG )
    {
        UDISK_OutPackflag = 1;
    }
    ...
    {                                
        if( packlen == 0x1F )
        {
            ...
            /* perform scsi command package processing */
            UDISK_SCSI_CMD_Deal( );
...
* Input          : Transfer_DataLen--- the length of the data transferred
*                  *pBuf---data address pointer
* Output         : Transfer_DataLen--- the length of the data transferred
*                  *pBuf---data address pointer
* Return         : None
*******************************************************************************/
...
{                                            
    ...
    /* load the data into the upload buffer and initiate the upload */
    if( Link_Sta == LINK_STA_1 )
    {
...
* Description    : bulk endpoint endpoint 2 uploads csw packages
* Input          : CBW_Tag_Save---command block labels
*                  CSW_Status---status of the current command execution result
* Output         : Bit_DISK_CSW---the csw package logo has not been uploaded
*                  Bit_DISK_Bluck_Up---data upload flag
*                  Bit_DISK_Bluck_Down---data downlink flag
* Return         : None
*******************************************************************************/
...
*
* Description    : batch endpoint endpoint transfer packet processing guaranteed transfer speed usb interrupts off emmc and usb simultaneous transfer processing
*
* Return         : None
*******************************************************************************/
...
* Description    : udisk uploads a packet of data
*                  this function is used when the usb flash drive reads the sector 
*                  command and after uploading a packet of data each time it will be 
*                  interrupted again as needed consider whether to proceed with the upload
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void UDISK_Up_OnePack( void )
{
    ...
    /* upload the data of this package */
    if( Link_Sta == LINK_STA_1 )
    {
    ...
    /* check whether all the data has been uploaded */    
...
* Description    : udisk handles a packet of down transmitted data
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
