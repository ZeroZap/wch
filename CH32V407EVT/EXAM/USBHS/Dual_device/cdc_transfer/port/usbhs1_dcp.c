...
usb_rst_e usbhs1_dch_init(usb_bool_t interrupt)
{
    ...
    usbhs1d_handle.feature.burst_support = USB_TRUE;
    ...
    usbhs1d_handle.set_endp_status = _set_endp_status;
    usbhs1d_handle.set_endp_size = _set_endp_size;
    usbhs1d_handle.set_endp_buf = _set_endp_buf;
    usbhs1d_handle.set_endp_burst_size = _set_endp_burst_size;
    usbhs1d_handle.get_endp_status = _get_endp_status;
    usbhs1d_handle.get_endp_size = _get_endp_size;
    usbhs1d_handle.get_endp_buf = _get_endp_buf;
    usbhs1d_handle.get_endp_burst_size = _get_endp_burst_size;
...
static usb_rst_e _enable(void)
{
    ...
    USBHS1D->UEP_TX_EN = 0;
    USBHS1D->UEP_RX_EN = 0;
    USBHS1D->BASE_MODE = USBHS_UD_SPEED_HIGH;
...
static usb_rst_e _get_event(usbd_event_t *e)
{
    ...
    if (flag & USBHS_UDIF_TRANSFER)
    {
        ...
        else if (dir)
        {
            ...
            e->xfer.buf = (endp == 0x00) ? (void *)USBHS1D->UEP0_DMA : (void *)ENDP_TX_DMA_ADDR(endp);
        }
        else if (ENDP_RX_CTRL(endp) & USBHS_UEP_R_TOG_MATCH)
        {
            ...
            ENDP_RX_LEN(endp) = 0;
            e->xfer.buf = (endp == 0x00) ? (void *)USBHS1D->UEP0_DMA : (void *)ENDP_RX_DMA_ADDR(endp);
        }
        else
        {
    ...
    {
        e->e = USBD_EVENT_SOF;
        e->sof.frame_no = USBHS1D->FRAME_NO & 0x07FF;
    }
    else if (flag & USBHS_UDIF_BUS_RST)
    {
        e->e = USBD_EVENT_RESET;
        Delay_Ms(10);
        e->reset.link_speed = (USBHS1D->MIS_ST & USBHS_UDMS_HS_MOD) ? USB_SPEED_HIGH : USB_SPEED_FULL;
    }
